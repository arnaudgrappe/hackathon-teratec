...
const ui64 order = 8;
ui64 DIMX, DIMY, DIMZ, DIMZ_cropped, iters;
ui64 MAXX, MAXY, MAXZ, MAXZ_cropped;
ui64 xyplane, MATsize, MATsize_cropped;
...
void init_parent() {
    ...
    matA = new double[MATsize_cropped];
    assert(matA != NULL);
    matB = new double[MATsize_cropped];
    assert(matB != NULL);
    matC = new double[MATsize_cropped];
    matC_final = new double [MATsize];

    assert(matC != NULL);

    MAXZ_cropped = (MATsize / Number_of_processes) + 2 * order;
