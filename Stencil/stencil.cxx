    ...
    for (ui64 z = 0; z < DIMZ; z++) {
        for (ui64 y = 0; y < DIMY; y++) {
            for (ui64 x = 0; x < DIMX; x++) {
                ...
                for (ui64 o = 1; o <= order; o++) {
                    matC[DIMXYZ(x, y, z)] += matA[DIMXYZ(x + o, y, z)] * matB[DIMXYZ(x + o, y, z)] / power_list[o-1];
                    matC[DIMXYZ(x, y, z)] += matA[DIMXYZ(x - o, y, z)] * matB[DIMXYZ(x - o, y, z)] / power_list[o-1];
                }
            }
        }
        ...
        for (ui64 x = 0; x < DIMX; x++) {
            for (ui64 y = 0; y < DIMY; y++) {
                matC[DIMXYZ(x, y, z)] = matA[DIMXYZ(x, y, z)] * matB[DIMXYZ(x, y, z)];
                for (ui64 o = 1; o <= order; o++) {
                    matC[DIMXYZ(x, y, z)] += matA[DIMXYZ(x, y + o, z)] * matB[DIMXYZ(x, y + o, z)] / power_list[o-1];
                    matC[DIMXYZ(x, y, z)] += matA[DIMXYZ(x, y - o, z)] * matB[DIMXYZ(x, y - o, z)] / power_list[o-1];
                }
            }
        }
    ...
    for (ui64 y = 0; y < DIMY; y++) {
        for (ui64 x = 0; x < DIMX; x++) {
            for (ui64 z = 0; z < DIMZ; z++) {
                matC[DIMXYZ(x, y, z)] = matA[DIMXYZ(x, y, z)] * matB[DIMXYZ(x, y, z)];
                for (ui64 o = 1; o <= order; o++) {
                    matC[DIMXYZ(x, y, z)] += matA[DIMXYZ(x, y, z + o)] * matB[DIMXYZ(x, y, z + o)] / power_list[o-1];
                    matC[DIMXYZ(x, y, z)] += matA[DIMXYZ(x, y, z - o)] * matB[DIMXYZ(x, y, z - o)] / power_list[o-1];
                }
            }
        }
